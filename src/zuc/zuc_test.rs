...
fn zuc_core() {
    ...
    let cases = [
        (
            [0x00u8,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00u8,0x00,0x00,0x00,0x00,0x00,0x00,0x00], 
            [0x00u8,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00u8,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
            [0x27bede74u32, 0x018082da],
        ),
        (
            [0xffu8,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xffu8,0xff,0xff,0xff,0xff,0xff,0xff,0xff],
            [0xffu8,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xffu8,0xff,0xff,0xff,0xff,0xff,0xff,0xff],
    ...
    let mut zuc = ZUC::from_slice((vec![0;16]).as_slice(), (vec![0;16]).as_slice()).unwrap();
    cases.iter().for_each(|ele| {
        zuc.set(ele.0, ele.1);
        for &e in (ele.2).iter() {
            assert_eq!(e, zuc.next().unwrap(), "case: key->{:?}", ele.0);
        }
    });
}
...
fn zuc_cipher() {
    ...
    let cases = [
        Cases {
            ck: [0x17, 0x3d, 0x14, 0xba, 0x50, 0x03, 0x73, 0x1d,
                0x7a, 0x60, 0x04, 0x94, 0x70, 0xf0, 0x0a, 0x29],
            count: 0x66035492,
            bearer: 0xf,
            direction: 0,
            bits_len: 0xc1,
            ibs: vec![0x6cf65340, 0x735552ab, 0x0c9752fa, 0x6f9025fe,
                      0x0bd675d9, 0x005875b2, 0x00000000,],
            obs: vec![0xa6c85fc6, 0x6afb8533, 0xaafc2518, 0xdfe78494,
                              0x0ee1e4b0, 0x30238cc8, 0x00000000, ],
        },
        Cases {
            ck: [0xe5, 0xbd, 0x3e, 0xa0, 0xeb, 0x55, 0xad, 0xe8,
                0x66, 0xc6, 0xac, 0x58, 0xbd, 0x54, 0x30, 0x2a],
            count: 0x56823,
            bearer: 0x18,
            direction: 1,
            bits_len: 0x320,
            ibs: vec![0x14a8ef69, 0x3d678507, 0xbbe7270a, 0x7f67ff50,
                      ...
                      0x01ba220d],
            obs: vec![0x131d43e0, 0xdea1be5c, 0x5a1bfd97, 0x1d852cbf,
        ...
        Cases {
            ck: [0xe1, 0x3f, 0xed, 0x21, 0xb4, 0x6e, 0x4e, 0x7e,
                0xc3, 0x12, 0x53, 0xb2, 0xbb, 0x17, 0xb3, 0xe0],
            count: 0x2738cdaa,
            bearer: 0x1a,
            direction: 0,
            bits_len: 0xfb3,
            ibs: vec![0x8d74e20d, 0x54894e06, 0xd3cb13cb, 0x3933065e,
            ...
            obs: vec![0x94eaa4aa, 0x30a57137, 0xddf09b97, 0xb25618a2,
    ...
    for (i, ele) in cases.iter().enumerate() {
        buf.clear();
        tmp.clear();
        let zuc_cipher = ZUCCipher::new(ele.count, ele.bearer, ele.direction == 1, ele.ck);
        ele.ibs.iter().for_each(|&e| {
            e.to_be_bytes().iter().for_each(|&x| {tmp.push(x);});
        });
        zuc_cipher.encrypt(&mut buf, tmp.as_slice()).unwrap();
        
        tmp.clear();
        ele.obs.iter().for_each(|&e| {
            e.to_be_bytes().iter().for_each(|&x| {tmp.push(x);});
        });
        buf.iter().zip(tmp.iter()).enumerate().for_each(|(j, (&a, &b))| {
            if j < ((ele.bits_len as usize) >> 3) {
                assert_eq!(a, b, "case: {}", i);
            } else if j < ((ele.bits_len as usize + 7) >> 3) {
                let mask = 8 - ((ele.bits_len as usize) - (j << 3));
                assert_eq!(a >> mask, b >> mask, "case: {}", i);
            } else {
            }
        });
    }
...
fn zuc_mac() {
    ...
    let cases = [
        Cases {
            ik: [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        Cases {
            ik: [0xc9, 0xe6, 0xce, 0xc4, 0x60, 0x7c, 0x72, 0xdb,
                0x00, 0x0a, 0xef, 0xa8, 0x83, 0x85, 0xab, 0x0a],
            count: 0xa94059da, 
            bearer: 0xa,
            direction: 1,
            // bits_len: 0x241,
            msg: vec![0x983b41d4, 0x7d780c9e, 0x1ad11d7e, 0xb70391b1,
        ...
        Cases {
            ik: [0x6b, 0x8b, 0x08, 0xee, 0x79, 0xe0, 0xb5, 0x98,
                0x2d, 0x6d, 0x12, 0x8e, 0xa9, 0xf2, 0x20, 0xcb],
            count: 0x561eb2dd,
            bearer: 0x1c,
            direction: 0,
            // bits_len: 0x1626,
            msg: vec![0x5bad7247, 0x10ba1c56, 0xd5a315f8, 0xd40f6e09,
    ...
    for (i, ele) in cases.iter().enumerate() {
        buf.clear();
        tmp.clear();
        let mut zuc_mac = ZUCMac::new(ele.count, ele.bearer, ele.direction == 1, ele.ik);
        ele.msg.iter().for_each(|&x| {
            x.to_be_bytes().iter().for_each(|&y| {tmp.push(y);});
        });
        zuc_mac.write(tmp.as_slice());
        zuc_mac.checksum(&mut buf);
